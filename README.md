...
- Code quality: S.O.L.I.D + DRY are required design principles; prefer readable, semantically dense code within the style rules and avoid unnecessary helper methods. ([Code Quality](./docs/php-style-guide.md#code-quality))
...
- Methods and parameters: strict types, avoid `mixed`, nullable only when needed, optional params last, no boolean flag params, and focused methods without unnecessary helpers. ([Methods and Parameters](./docs/php-style-guide.md#methods-and-parameters))
...
- Comments/PHPDoc: explain “why,” use PHPDoc for public non-trivial APIs or generic collections, include `@param`/`@return` for generic iterables and `@throws` for exceptions, prefer precise annotations over helper methods when runtime safety is not weakened, and avoid redundant docblocks. ([Comments and PHPDoc](./docs/php-style-guide.md#comments-and-phpdoc))
